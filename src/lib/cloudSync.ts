...
export async function pullCloudState(local: RecoveryState): Promise<RecoveryState> {
  ...
  const [profile, checkIns, urges, relapses, journal, goals, challenges, streaks, programProgress, achievements] = await Promise.all([
    supabase.from("profiles").select("id, display_name, goal_days, timezone, primary_goal, preferred_approach, common_triggers, onboarding_complete, reminders_enabled, ai_storage_enabled, created_at").eq("id", user.id).maybeSingle(),
  ...
  return {
    ...
    journalEntries: mergeById(local.journalEntries, (journal.data ?? []).map(fromJournal), "updatedAt"),
